...
const initialHead = (heads) => {
  const domString = `<img class='card-img-top head-img' src=${heads[0].imageUrl}>`;
  $('#heads').html(domString);
  $('#head-name').html(heads[0].name);
};

const initialTorso = (torsos) => {
  const domString = `<img class='card-img-top torso-img' src=${torsos[0].imageUrl}>`;
  $('#torsos').html(domString);
  $('#torso-name').html(torsos[0].name);
};

const initialLeg = (legs) => {
  const domString = `<img class='card-img-top leg-img' src=${legs[0].imageUrl}>`;
  $('#legs').html(domString);
...
const displaySelectedHead = (clickedHead) => {
  let domString = '';
  domString += `<img class='card-img-top head-img' src=${clickedHead[0].imageUrl}>`;
  let nameString = '';
...
const displaySelectedTorso = (clickedTorso) => {
  let domString = '';
  domString += `<img class='card-img-top torso-img' src=${clickedTorso[0].imageUrl}>`;
  let nameString = '';
...
const displaySelectedLeg = (clickedLeg) => {
  let domString = '';
  domString += `<img class='card-img-top leg-img' src=${clickedLeg[0].imageUrl}>`;
  let nameString = '';
